...
class CKMS:
    ...
    def test(self,
             ...
             find_optimal_z : bool = False,
             scaling        : float = None) -> float:
        ...
        ones_t      = np.ones((t,1))
        r_f_bar     = r_f@ones_t/t
        x           = np.vstack([ones_t.T, r_f])
        omega_f     = r_f@r_f.T/t - r_f_bar@r_f_bar.T
        omega_f_inv = np.linalg.inv(omega_f)
        quad_f_form = r_f.T@omega_f_inv@r_f
        
        #* 1. Compute hat M(z), we also compute parameters estimate here.
        alpha_tilde = 1/t*ones_t*(1 + ones_t.T@quad_f_form@ones_t/t**2) - 1/t*quad_f_form@ones_t/t
        alpha_bar   = r@alpha_tilde
        beta_hat    = 1/t*r@r_f.T@omega_f_inv - 1/t*r@ones_t@r_f_bar.T@omega_f_inv
        
        B_hat = np.hstack([alpha_bar, beta_hat])
        
        #* Adjust the grid
        if adjust_grid:
            alpha_hat   = r - beta_hat@r_f
        ...
        #* 1.1 We compute the case where P > T, we use inverse directly because using eigenvalue decomposition would also imply needing to inverse Q and is in fact much slower.
        if p > t:
            ...
            hat_M_z = (1/(1+r_f_bar.T@omega_f_inv@r_f_bar))*alpha_tilde.T@tilde_sigma_inv@r.T@alpha_bar
            
        #* 1.2 We compute the case where P =< T, we use eigenvalue decomposition as it is needed later and therefore we reduce computational cost
        else:
            ...
            hat_sigma_inv = eigvecs@np.array([np.diag(1/(self.z[i] + eigvals)) for i in range(len(self.z))])@eigvecs.T
            
            hat_M_z = (1/(1+r_f_bar.T@omega_f_inv@r_f_bar))*alpha_bar.T@hat_sigma_inv@alpha_bar
            
        #* 2. Compute hat m(z)
        #* 2.1 We handle the case where P > T
        if p > t:
            hat_m_z = 1/p*np.array([np.trace(tilde_sigma_inv[i]) for i in range(len(self.z))]) + (p-t)/(p*np.array(self.z))
        ...
        #* 3. Compute hat m'(z)
        if p > t:
            hat_m_prime_z = 1/p*np.array([np.trace(tilde_sigma_inv[i]@tilde_sigma_inv[i]) for i in range(len(self.z))]) + (p-t)/(p*np.array(self.z)**2)
        else: 
        ...
        best_z_ind = None
        if find_optimal_z:
            h_zc       = self.get_estimated_h(r, r_f)
    ...
    def summary(self):
        ...
        if self.test_results['best_z_ind'] is None:
            ...
            table = [
                ...
                ["Test Statistic", str(np.round(self.test_results['test_stat'],2)), str(np.round(self.test_results['test_stat'][0,self.test_results['best_z_ind']],2))],
                ["P-Value", str(np.round(self.test_results['p_value'],2)), str(np.round(self.test_results['p_value'][0,self.test_results['best_z_ind']], 2))]
            ]
